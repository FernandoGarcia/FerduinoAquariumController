...
void RFM12B::
Initialize(uint8_t ID, uint8_t freqBand, uint8_t networkid, uint8_t txPower, uint8_t airKbps, uint8_t lowVoltageThreshold)
{
  //while(millis()<60);
  cs_pin = SS_BIT;
  nodeID = ID;
  ...
  else
  {
    ...
    else
    {
      switch (rxstate++)
      {
        ...
        case TXSYN2:
          {
            out = networkID; rxstate = -(3 + rf12_len);
          }
          break;
        case TXCRC1:
          {
...
CanSend()
{
  /*Serial.print("Byte(0x00) & (RF_RSSI_BIT >> 8): ");
     Serial.println(Byte(0x00) & (RF_RSSI_BIT >> 8));*/
...
void RFM12B::
SendStart(uint8_t toNodeID, bool requestACK, bool sendACK)
{
...
void RFM12B::
SendStart(uint8_t toNodeID, const void* sendBuf, uint8_t sendLen, bool requestACK, bool sendACK, uint8_t waitMode)
{
  rf12_len = sendLen;
...
void RFM12B::
SendACK(const void* sendBuf, uint8_t sendLen, uint8_t waitMode)
{
  while (!CanSend())
  {
    ReceiveComplete();
...
void RFM12B::
Send(uint8_t toNodeID, const void* sendBuf, uint8_t sendLen, bool requestACK, uint8_t waitMode)
{
  while (!CanSend())
  {
    ReceiveComplete();
...
bool RFM12B::
ACKReceived(uint8_t fromNodeID)
{
  if (ReceiveComplete())
  {
    return CRCPass() &&
...
void RFM12B::
Encrypt(const uint8_t* key, uint8_t keyLen)
{
