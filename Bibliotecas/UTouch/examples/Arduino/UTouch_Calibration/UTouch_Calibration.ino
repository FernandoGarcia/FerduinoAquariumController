...
// Open the serial monitor to get the coordinates.
// After do the process go to http://www.rinkydinkelectronics.com/t_cal_verify.php to check the coordinates.
// If all is right place this values at UtouchCD.h

#include <UTFT.h>
#include <UTouch.h>

#define TOUCH_ORIENTATION  PORTRAIT
//#define TOUCH_ORIENTATION  LANDSCAPE // Try this option if you haven't good results with PORTRAIT orientation

extern uint8_t SmallFont[];

UTFT        myGLCD(ITDB32WD,38,39,40,41);   // ITDB32WD is the model of your TFT.
//UTouch      myTouch(6,5,4,3, 2); // For TFT shield
...
{
  Serial.begin(9600);
  Serial.println(F("After do the process go to http://www.rinkydinkelectronics.com/t_cal_verify.php to check the coordinates."));
  Serial.println(F("If all is right place this values at UtouchCD.h"));
  myGLCD.InitLCD();
  myGLCD.clrScr();
